...
const officersModel = {
    ...
    addNewOfficer: thunk(async (actions, payload, { getState, getStoreState }) => {
        ...
        const [addNewOfficerData, addNewOfficerError] = await handlePromise(officersAPI.addNewOfficer(token, payload));
    ...
    editOfficer: thunk(async (actions, payload, { getState, getStoreState }) => {
        ...
        const [editOfficerData, editOfficerError] = await handlePromise(officersAPI.editOfficer(token, payload.officerId, payload.officerData));
    ...
    deleteOfficer: thunk(async (actions, payload, { getState, getStoreState }) => {
        const token = getStoreState().auth.authState.token;
        const [deleteOfficerData, deleteOfficerError] = await handlePromise(officersAPI.deleteOfficer(token, payload.officerId));
